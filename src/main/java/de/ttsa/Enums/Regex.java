package de.ttsa.Enums;

public enum Regex {
    SAY_OPCODE("^((\"([a-zA-Z0-9\\s\\?\\!])+\")|([a-zA-Z]+[a-zA-Z0-9]*|[\\d]?)){1}([,]((\"([\\w\\s\\?\\!]?)+\")|([a-zA-Z0-9]+)+))*"), 
    ROOM_OPCODE("([a-zA-Z][\\w\\s]*:[\\d]+)"), 
    ROOM_JUMPER_OPCODE("([a-zA-Z]\\w*)"), 
    NUMBER_DEC_OPCODE("([a-zA-Z][\\w]*:[\\d]+)"), 
    STR_DEC_OPCODE("([a-zA-Z][\\w]*:[\\w\\s\\!\\?]*)"), 
    IF_NUMBER_OPCODE("(^(([a-zA-Z]+[\\w]*)|([\\d]*))([\\<]|[\\>]|[\\<][\\=]|[\\>][=]|[\\=][\\=]|[\\!][\\=])(([a-zA-Z]+[\\w]*)|([\\d]*)){1}(([\\+\\-\\*\\/])(([a-zA-Z]+[\\w]*)|([\\d]*)))*)"), 
    IF_INNER_BRECKETS_OPCODE("(-)?(([a-zA-Z]+[\\w]*)|([\\d]))*(([\\+\\-\\*\\/]){1}(-)?(([a-zA-Z]+[\\w]*)|([\\d])))*"), 
    IF_STRING_OPCODE("(([a-zA-Z]+[\\w]*)|(\"[a-zA-Z]+[\\w]*)\")((\\=\\=)|(\\!\\=))(([a-zA-Z]+[\\w]*)|(\"[a-zA-Z]+[\\w]*)\")"), 
    IF_INPUT_OPCODE("(~?-?\\[(-?\\{(-?(\\\"[\\w.,!?]*\\\"|'[a-zA-Z][\\w]*'|@[a-zA-Z][\\w]*@)\\*\\(\\d*\\))(-?(\\\"[\\w.,!?]*\\\"|'[a-zA-Z][\\w]*'|@[a-zA-Z][\\w]*@)\\*\\(\\d*\\))*\\}\\*\\(\\d*\\))(-?\\{(-?(\\\"[\\w.,!?]*\\\"|'[a-zA-Z][\\w]*'|@[a-zA-Z][\\w]*@)\\*\\(\\d*\\))(-?(\\\"[\\w.,!?]*\\\"|'[a-zA-Z][\\w]*'|@[a-zA-Z][\\w]*@)\\*\\(\\d*\\))*\\}\\*\\(\\d*\\))*\\]\\*\\(\\d*\\))(~?-?\\[(-?\\{(-?(\\\"[\\w.,!?]*\\\"|'[a-zA-Z][\\w]*'|@[a-zA-Z][\\w]*@)\\*\\(\\d*\\))(-?(\\\"[\\w.,!?]*\\\"|'[a-zA-Z][\\w]*'|@[a-zA-Z][\\w]*@)\\*\\(\\d*\\))*\\}\\*\\(\\d*\\))(-?\\{(-?(\\\"[\\w.,!?]*\\\"|'[a-zA-Z][\\w]*'|@[a-zA-Z][\\w]*@)\\*\\(\\d*\\))(-?(\\\"[\\w.,!?]*\\\"|'[a-zA-Z][\\w]*'|@[a-zA-Z][\\w]*@)\\*\\(\\d*\\))*\\}\\*\\(\\d*\\))*\\]\\*\\(\\d*\\))*"), 
    STR_VARDEC_OPCODE("([a-zA-Z]\\w*)\\:(([a-zA-Z]\\w*)|(\"[\\w\\s\\?\\!]*\"))(\\,([a-zA-Z]\\w*)|\\,(\"[\\w\\s\\?\\!]*\"))*"), 
    DEBUG_OPCODE("(([a-zA-Z]\\w*)|(\"[\s]?([a-zA-Z][\\w\\s]*)?\"))(,(\"[\s]?([a-zA-Z][\\w\\s]*)?\")|,([a-zA-Z]\\w*))*"), 
    EXIT_OPCODE("[0]|[1]"), 
    LOOP_OPCODE("(true)|\\d+|([a-zA-Z][\\w]*)"), 
    SET_OPCODE("^([a-zA-Z][\\w]*)[\\:]([\"][a-zA-Z][\\w]*[\"]|[a-zA-Z][\\w*])([\\,]([\"][a-zA-Z][\\w]*[\"]|[a-zA-Z][\\w]*))*"), 
    ACTION_OPCODE("([a-zA-Z]\\w*\\:)((-)|([sn][a-zA-Z]\\w*)([\\,]([sn][a-zA-Z]\\w*))*)(\\:\\d*)"), 
    ACTION_CALL_OPCODE("([a-zA-Z]\\w*\\:)((-)|([a-zA-Z]\\w*)|(\"([\\w\\s])*\")|\\d*)(\\,((-)|([a-zA-Z]\\w*)|(\"([\\w\\s])*\")|\\d*))*"), 
    MUSIC_DEC_OPCODE("[a-zA-Z]\\w*:[\\w]+.?((wav)|(mp3))"),
    MUSIC_STARTER_OPCODE("[a-zA-Z]\\w*(,[a-zA-Z]\\w*)*"),

    SAY_CODE("('[a-zA-Z]\\w*'|[\\w\\s!?]*)*"), 
    ROOM_CODE("\s*[a-zA-Z]\\w*\\s*\\{"), 
    ROOM_JUMPER_CODE("\s*[a-zA-Z]\\w*\\s*"), 
    NUMBER_DEC_CODE("[a-zA-Z]\\w*\\s*(=\\s*)?[\\d]*\\s*"),
    STR_DEC_CODE("[a-zA-Z]\\w*\\s*(=\\s*)?[\\w]*\\s*"),
    IF_NUMBER_CODE("(^(([a-zA-Z]+[\\w]*)|([\\d]*))([\\<]|[\\>]|[\\<][\\=]|[\\>][=]|[\\=][\\=]|[\\!][\\=])(([a-zA-Z]+[\\w]*)|([\\d]*)){1}(([\\+\\-\\*\\/])(([a-zA-Z]+[\\w]*)|([\\d]*)))*)"), 
    IF_INNER_BRECKETS_CODE("(-)?(([a-zA-Z]+[\\w]*)|([\\d]))*(([\\+\\-\\*\\/]){1}(-)?(([a-zA-Z]+[\\w]*)|([\\d])))*"), 
    IF_STRING_CODE("(([a-zA-Z]+[\\w]*)|(\"[a-zA-Z]+[\\w]*)\")((\\=\\=)|(\\!\\=))(([a-zA-Z]+[\\w]*)|(\"[a-zA-Z]+[\\w]*)\")"), 
    IF_INPUT_CODE("((\\s*-?\\s*(\"\\w*\"|@[a-zA-Z]\\w*|'[a-zA-Z]\\w*')(\\s*\\*(\\s*\\(\\s*\\d*\\s*\\)\\s*|\\s*\\d*\\s*))?)*|(\\s*-?\\s*\\{\\s*(\\s*-?\\s*(\"\\w*\"|@[a-zA-Z]\\w*|'[a-zA-Z]\\w*')(\\s*\\*(\\s*\\(\\s*\\d*\\s*\\)\\s*|\\s*\\d*\\s*))?)*\\s*\\}\\s*(\\s*\\*(\\s*\\(\\s*\\d*\\s*\\)\\s*|\\s*\\d*\\s*))?)*|(\\s*-?\\s*\\[\\s*(\\s*-?\\s*(\"\\w*\"|@[a-zA-Z]\\w*|'[a-zA-Z]\\w*')(\\s*\\*(\\s*\\(\\s*\\d*\\s*\\)\\s*|\\s*\\d*\\s*))?)*|(\\s*-?\\s*\\{\\s*(\\s*-?\\s*(\"\\w*\"|@[a-zA-Z]\\w*|'[a-zA-Z]\\w*')(\\s*\\*(\\s*\\(\\s*\\d*\\s*\\)\\s*|\\s*\\d*\\s*))?)*\\s*\\}\\s*(\\s*\\*(\\s*\\(\\s*\\d*\\s*\\)\\s*|\\s*\\d*\\s*))?)*\\s*\\]\\s*(\\s*\\*(\\s*\\(\\s*\\d*\\s*\\)\\s*|\\s*\\d*\\s*))?)*)+"), 
    STR_VARDEC_CODE("\\s*[a-zA-Z]\\w*\\s*=\\s*((\\'[a-zA-Z]\\w*\\')|(\\w*))\\s*"), 
    DEBUG_CODE("('[a-zA-Z]\\w*'|[\\w\\s!?]*)*"), 
    EXIT_CODE(""), 
    LOOP_CODE(""), 
    SET_CODE("\\s*[a-zA-Z]\\w*\\s*\\{"), 
    SET_CONTENT("\\s*((\\w*)|('[a-zA-Z]\\w*'))\\s*"),
    ACTION_CODE("[a-zA-Z]\\w*\\s*\\(((\\s*)|((\\s*((Num)|(Str))\\s+[a-zA-Z]\\w*\\s*)(,(\\s*((Num)|(Str))\\s+[a-zA-Z]\\w*\\s*))*))\\)\\s*\\{"), 
    ACTION_CALL_CODE("\\s*[a-zA-Z]\\w*\\s*(([a-zA-Z]\\w*)|(\"[a-zA-Z]\\w*\")|(\\d+))\\s*(,\\s*(([a-zA-Z]\\w*)|(\"[a-zA-Z]\\w*\")|(\\d+)))?\\s*"),
    MUSIC_DEC_CODE("\\s*[a-zA-Z]\\w*\\s*=\\s*[\\w]+.?((wav)|(mp3))\\s*"),
    MUSIC_STARTER_CODE("(\\s*[a-zA-Z]\\w*)*\\s*"),


    VALIDE_NAME("([a-zA-Z][\\w]*)"), 
    VALIDE_NUMBER("^[-]?[0-9]+$"), 
    CALCULATABLE("^(([-]?([0-9]))*|([-]?([a-zA-Z]+[a-zA-Z0-9])))*(?:[-+*/][-]?[a-zA-Z0-9]+)*$");

    private String command;

    Regex(String regex) {
        this.command = regex;
    }

    @Override
    public String toString() {
        return command;
    }
}
