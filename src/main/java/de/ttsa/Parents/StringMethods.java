...
public class StringMethods {
    ...
    protected String deleteFirst(String identifier, String string) {
        return string.substring(identifier.length());
    }

    protected String getFirstNumbers(String string) {
        int index = 0;
    ...
    protected String deleteFirstNumbers(String string) {
        int index = 0;
        for(int i = 0; i < string.length(); i++) {
            if(Character.isDigit(string.charAt(i))) {
                index++;
    ...
    protected char getFirstChar(String toFind, char... chars) {
        int first = -1;
        char result = 0;
        for(char c : chars) {
            if(toFind.indexOf(c) != -1) {
                if(first == -1) {
                    first = toFind.indexOf(c);
                    result = c;
                } else {
                    if(toFind.indexOf(c) < first) {
                        first = toFind.indexOf(c);
                        result = c;
                    }
                }
